...
void main() {	
	...
	vec3 light = texture(specularTexture,vert.uv).rgb;
	texColor = texColor*light;
	...
	for(int i=0;i<4;i++)
	{
		...
		shadowDir += texture(shadowMap,shadowUv+poissonDisk[i]/800.0f)/4; //non random no noise but jaggies
